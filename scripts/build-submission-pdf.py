...
# ---------------------------------------------------------------- 증거 3b: 실기기
y = newpage("EVIDENCE · C26", "실물 기기에서 확인했습니다",
            "가상 인증기가 아니라 실제 휴대폰으로 등록하고, 로그아웃한 뒤 같은 패스키로 다시 들어갔습니다.")
PH_W = (CW - 24) / 2
h1 = img_height(SHOTS / "phone-01-passkey-list.png", PH_W)
image(SHOTS / "phone-01-passkey-list.png", M, y, PH_W)
image(SHOTS / "phone-02-google-manager.png", M + PH_W + 24, y, PH_W)
y -= h1 + 12
para("<b>목록 화면</b> — 내 휴대폰 · 2026-09-11 · <b>동기화됨 · 이 기기</b>",
     M, y, PH_W, 8.5, "KR", C["ink"], 12)
para("<b>Google 비밀번호 관리자</b> — 등록·로그인 때 뜨는 창",
     M + PH_W + 24, y, PH_W, 8.5, "KR", C["ink"], 12)
y -= 26
card(M, y, CW, 92)
para("<b>두 화면이 같은 것을 가리킵니다</b>", M + 12, y - 10, CW - 24, 9.5, "KRB", C["forest"])
para("왼쪽 목록의 <b>동기화됨</b>은 구글 계정에 백업되는 자격증명이라는 뜻이고, 오른쪽 창이 그 보관처가 "
     "<b>Google 비밀번호 관리자</b>임을 확인해 줍니다. 기기 자체도 보안 키도 아닙니다. "
     "심사하는 사람이 기기 설정을 열지 않아도 웹 화면만으로 저장 위치를 알 수 있고, 그 진술이 "
     "운영체제의 자격증명 목록과 어긋나지 않는다는 것이 이 두 장으로 확인됩니다. (T08-C26)",
     M + 12, y - 28, CW - 24, 8.5, "KR", C["muted"], 12.5)
y -= 104
para("오른쪽 창이 표시하는 계정명은 <font name=\"MONO\">synthetic-</font> 으로 시작하는 합성 식별자입니다. "
     "표시 이름을 합성값으로 둔 결정이 운영체제의 자격증명 목록까지 이어져, 실명이 들어가지 않았다는 사실이 "
     "서버 기록만이 아니라 기기 화면으로도 확인됩니다.",
     M, y, CW, 8.5, "KR", C["muted"], 12.5)
footer("실물 안드로이드 기기 · 크롬 · 2026-09-11 · 상단 알림줄과 하단 내비게이션 바는 잘라냈습니다")
