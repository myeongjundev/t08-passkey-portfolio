...
- 등록 직후 `/private` 첫 요청이 간헐적으로 500이 될 수 있습니다. 2026-09-11 실물
  휴대폰 등록에서 한 번 관측했고, 새로고침하면 열립니다. 등록 자체는
  `@Transactional`로 이미 커밋된 뒤라 데이터는 정상이지만, **원인은 확정하지
  못했습니다.** Vercel 실행 로그를 확보하지 못했고 재현도 되지 않았습니다. 지금
  대응은 원인 제거가 아니라 `templates/error.html`로 실패를 읽을 수 있게 만든
  것까지입니다. 기록은 `docs/evidence/12-real-device-registration.md`에 있습니다.
